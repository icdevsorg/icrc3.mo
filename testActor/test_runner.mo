...
import Nat "mo:core/Nat"; // Adding Nat for range helper
...
shared(init_msg) persistent actor class() = this {
  ...
  public func testIntegrityOfLedgerBlocks() : async { #success; #fail : Text } {
    ...
    let _index1 = await ledger.add_record(transaction1);
    let _index2 = await ledger.add_record(transaction2);
    ...
    let _trxresult1 = ICRC3.helper.get_item_from_map("tx", retrievalResult.blocks[0].block);
    let _trxresult2 = ICRC3.helper.get_item_from_map("tx", retrievalResult.blocks[1].block);

    //D.print()

    let ?#Blob(derived) = ICRC3.helper.get_item_from_map("phash", retrievalResult.blocks[1].block) else Runtime.trap("expected phash");
  ...
  public func testTipCertificationMatchesLastBlock() : async { #success; #fail : Text } {
    let ledger = await (with cycles = 10_000_000_000_000) Example.Example(baseState);
    ...
    let _index = await ledger.add_record(transaction);
    ...
    public shared func testArchivedBlocksCallbackReturnsCorrectData() : async  { #success; #fail : Text }{
      ...
      for(thisItem in range(0, 10)){
        ...
        let _fake1 = await Fake.Fake();
        D.print("faking ");
        let _fake2 = await Fake.Fake();
        let _fake3 = await Fake.Fake();
        let _fake4 = await Fake.Fake();
    ...
    let _transaction3 : ICRC3.Transaction = #Map([
        ("op", #Text("transfer")),
        ("from", #Text("alice")),
        ("to", #Text("bob")),
        ("amount", #Nat(230)),
    ...
    let _index3 = await ledger.add_record(transaction2);
    ...
    let ?#Blob(secondBlockHash) = ICRC3.helper.get_item_from_map("phash", retrievalResult.blocks[1].block) else Runtime.trap("expected phash");

    let ?#Blob(thirdBlockHash) = ICRC3.helper.get_item_from_map("phash", retrievalResult.blocks[2].block) else Runtime.trap("expected phash");

    
    let blockTxHash = Blob.fromArray(RepIndy.hash_val(#Map([("tx",transaction1)])));

    let _expectedHash2 = Blob.fromArray(RepIndy.hash_val(retrievalResult.blocks[0].block));
    ...
    public func testRetrieveBlockLog() : async { #success; #fail : Text } {
      ...
      let ?trxresult1 = ICRC3.helper.get_item_from_map("tx",retrievalResult.blocks[0].block) else Runtime.trap("not a map");

      let ?trxresult2 = ICRC3.helper.get_item_from_map("tx",retrievalResult.blocks[1].block)  else Runtime.trap("not a map");
    ...
    public shared func testUpgrade() : async { #success; #fail : Text } {
        ...
        let childv1 = await (with cycles = 10_000_000_000_000) Example.Example(baseState);
        ...
        let _addRoles1 = await childv1.addRole("test1");
        D.print("have a log result " # debug_show(_addRoles1));
        let _addRoles2 = await childv1.addRole("test1a");
        D.print("have a log result " # debug_show(_addRoles2));
        let _addRoles3 = await childv1.addRole("test1b");
        let _addRoles4 = await childv1.addRole("test1c");
        let _addRoles5 = await childv1.addRole("test1e");
        let _addRoles6 = await childv1.addRole("testf");
        let _addRoles7 = await childv1.addRole("test1g");
        let _addRoles8 = await childv1.addRole("test1h");
        let _addRoles9 = await childv1.addRole("test1i");
        let _addRoles10 = await childv1.addRole("test1j");
        let _addRoles11 = await childv1.addRole("test1k");
        let _addRoles12 = await childv1.addRole("test1l");
        let _addRoles13 = await childv1.addRole("test1m");
        let _addRoles14 = await childv1.addRole("test1n");
        let _addRoles15 = await childv1.addRole("test1o");
        let _addRoles16 = await childv1.addRole("test1p");
