/// ICRC3OVSTestCanisterV1 - Test canister for OVS cycle sharing verification
///
/// This canister wraps ICRC3 with OVS enabled (collector set) to test:
/// 1. OVS initializes correctly via OVSFixed
/// 2. Correct cycles are sent
/// 3. No duplicate OVS actions on stop/start
/// 4. No duplicate OVS actions on upgrade
///
/// Note: All OVS logic is handled by OVSFixed internally.
/// ICRC3 initializes OVSFixed in its wrappedOnInitialize function.
...
shared ({ caller = _owner }) persistent actor class ICRC3OVSTestCanisterV1(_args: ?{
  collector: ?Principal;
  period: ?Nat;
  initialWait: ?Nat;
}) = this {
  ...
  var _ovsPeriod : Nat = switch(_args) {
    case(?args) {
      switch(args.period) {
        case(?p) p;
        case(null) ONE_DAY; // 1 day for testing
      };
    };
    case(null) ONE_DAY;
  };
    ...
    pullEnvironment = ?(func() : TT.Environment {
      {      
        ...
        reportExecution = ?(func(execInfo: TT.ExecutionReport) : Bool {
          D.print("ICRC3OVSTestCanisterV1: execution " # debug_show(execInfo.action.1.actionType));
  ...
  transient let icrc3 = ICRC3.Init({
    ...
    onInitialize = ?(func(_instance: ICRC3.ICRC3) : async*(){
      D.print("ICRC3OVSTestCanisterV1: ICRC3 initialized");
  ...
  /// Add a test record to generate OVS activity
  ...
  D.print("ICRC3OVSTestCanisterV1: Actor initialized");
};
