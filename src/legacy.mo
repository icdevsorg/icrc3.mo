...
module {
  ...
  // Rosetta-compatible types for legacy get_blocks endpoint
  // These match the format expected by dfinity/ic-icrc-rosetta-api
  public type RosettaBlockRange = {
    blocks : [Value];
  };
  ...
  public type RosettaArchivedRange = {
    start : Nat;
    length : Nat;
    callback : shared query GetBlocksRequest -> async RosettaBlockRange;
  };

  public type RosettaGetBlocksResponse = {
    first_index : Nat;
    chain_length : Nat64;
    certificate : ?Blob;
    blocks : [Value];
    archived_blocks : [RosettaArchivedRange];
  };
